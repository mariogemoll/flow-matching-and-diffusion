...
export function setUpLabWidget(): void {
  const canvas1 = el(document, '#lab-canvas') as HTMLCanvasElement;
  const canvas2 = el(document, '#lab-canvas-2') as HTMLCanvasElement;
  const slider = el(document, '#animation-slider') as HTMLInputElement;
  const frameDisplay = el(document, '#frame-display') as HTMLElement;
  const playPauseButton = el(document, '#play-pause-button') as HTMLButtonElement;

  // State
  const state: LabState = {
    dotX: 400,  // Center of 800px canvas
    dotY: 300,  // Center of 600px canvas
    isDragging: false
  };
  ...
  function updateState(updates: Partial<LabState>): void {
